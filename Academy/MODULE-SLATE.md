...
| E2 | On **1Z0-083**, topics are grouped by source course. Counted from Oracle's own headings, twice, the second time programmatically after the first count was wrong: **Managing Multitenant Architecture 7, Backup and Recovery Workshop 9, Deploy Patch and Upgrade 8, 19c New Features 3, Administration Workshop 2**, total **29**. Multitenant plus backup and recovery are **16 of 29**. Performance is **2 of 29**, being "Monitoring and Tuning Database Performance" and "Tuning SQL Statements". | Same PDF, pages 8 to 12 | **High** for the topic lists. **Medium** for the group count, which is arithmetic over Oracle's headings and not an Oracle figure. Oracle publishes no weightings (E1), so a count of headings is a proxy and not a weight |
...
**What the split actually produces on the shelf:** Course B with three modules and Course A with two, **20 to 30 lessons total** at the four to six band in `MODULE-SPEC.md` 2.1. Both courses render as real courses, because `src/pages/academy/[course]/index.astro` already suppresses modules with no lessons in them (verified in code, `MODULE-SPEC.md` section 1).

### 5.3 The honest cost headline

**Five modules is 20 to 30 lessons**, at the four to six lessons per module band this spec sets. That band is the honest width; narrowing it to a tidier-looking number would be exactly the kind of confident figure `MODULE-SPEC.md` section 7 forbids. **No wall-clock estimate is given here on purpose**, because this seat has no measurement of what an Academy lesson costs to write.
